...
// Embed mode (PeopleAdvisory slider) has no color picker — instead each
// slide index maps to a fixed scheme here, alternating blue/purple/blue/...
const EMBED_SCHEME_CYCLE = ["blue", "purple"];
...
export default function Multiple3d({ embed = false, targetIndex = 0 }) {
  ...
  useEffect(() => {
    ...
    if (targetIndex !== prevIndexRef.current) {
      prevIndexRef.current = targetIndex;
      const scheme =
        EMBED_SCHEME_CYCLE[targetIndex % EMBED_SCHEME_CYCLE.length];
      goToShapeRef.current?.(targetIndex, scheme);
    }
  }, [targetIndex]);
  ...
  useEffect(() => {
    ...
    let currentShapeIndex = 0;
    const morphState = { progress: 0 };
    // Recolor mid-morph, once particles are scattered in the swirl/noise
    // phase, rather than at onComplete — swapping scheme on the crisp final
    // shape reads as a visible color "pop"; swapping while scattered hides
    // the change in the motion instead.
    let pendingColorScheme = null;
    const COLOR_SWAP_PROGRESS = 0.45;
    ...
    const COLOR_SCHEMES = {
      ...
      nature: { startHue: 90, endHue: 160, saturation: 0.85, lightness: 0.55 },
      rainbow: { startHue: 0, endHue: 360, saturation: 0.9, lightness: 0.6 },
      // Narrow hue bands (vs. the other schemes' wide sweeps) — these
      // models aren't spheres, so a point's distance-from-origin doesn't
      // line up with "visually central vs. edge." A wide hue sweep here
      // reads as the densest (visually central) mass landing on whichever
      // hue its actual distance happens to hit — e.g. blue's far end (230°)
      // drifting into indigo/violet territory and looking like a different
      // color entirely. Keeping the whole scheme inside one narrow band
      // means every point stays recognizably blue/purple regardless of
      // where in the model it falls.
      blue: { startHue: 210, endHue: 220, saturation: 0.85, lightness: 0.55 },
      purple: { startHue: 272, endHue: 282, saturation: 0.75, lightness: 0.55 },
    };
    ...
    // ── Jump to a specific shape index (scroll-driven) ────────────
    function morphToIndex(targetIdx, colorScheme) {
      if (targetIdx === currentShapeIndex) return;
      // Kill any running morph so we don't fight with it
      if (morphTimeline) morphTimeline.kill();
      isMorphing = true;
      // Queued — applied mid-morph by morphFrame() once particles are
      // scattered (see COLOR_SWAP_PROGRESS), not immediately.
    ...
    function morphFrame(positions, effectStrengths, elapsed, dt) {
      ...
      if (pendingColorScheme && t >= COLOR_SWAP_PROGRESS) {
        CONFIG.colorScheme = pendingColorScheme;
    ...
    // `embed`/`targetIndex` are read only for their mount-time value here
    // (initial camera FOV, morph duration, starting color scheme) — the
    // scroll-driven targetIndex changes are handled by the effect above via
    // goToShapeRef, not by re-running this whole three.js setup.
